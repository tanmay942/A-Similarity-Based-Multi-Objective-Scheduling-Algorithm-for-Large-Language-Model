...
MEMORY_DB_FILE = "memory_db_quality.json"
...
def select_model_from_neighbors_quality(neighbors: List[Tuple[float, Dict[str, Any]]]) -> str:
    # similarity-weighted score: sum(sim*score)/sum(sim)
    best = None
    best_wscore = float("-inf")
    ...
    for m in MODELS:
        ...
        for sim, row in neighbors:
            ...
            if isinstance(sc, (int, float)):
                wsum += float(sim)
                ssum += float(sim) * float(sc)
        if wsum <= 0.0:
            continue
        wscore = ssum / wsum
        if best is None or wscore > best_wscore:
            best = m
            best_wscore = wscore
...
# =========================
# MAIN: Decide first -> execute model-by-model (WITH CHECKPOINTS)
# =========================
def main():
    ...
    out_path = os.path.join(OUT_DIR, "baseline_always_quality.json")
    summary_path = os.path.join(OUT_DIR, "baseline_always_quality_summary.json")

    # NEW: extra safety files
    runs_path = os.path.join(OUT_DIR, "baseline_always_quality_runs_partial.json")
    planned_path = os.path.join(OUT_DIR, "baseline_always_quality_planned.json")

    # Resume judged results
    ...
    global_best = pick_global_best_quality(memory_db)
    ...
    for idx, item in enumerate(tests, start=1):
        ...
        if used_neighbors:
            selected_model = select_model_from_neighbors_quality(neighbors)
        else:
            selected_model = global_best
    ...
    for i, plan_entry in enumerate(planned, start=1):
        ...
        record = {
            "prompt_id": pid,
            "prompt": prompt,
            "baseline": "baseline_always_quality",
    ...
    summary = {
        ...
        "global_best_quality": global_best,
